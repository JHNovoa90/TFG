...
namespace Sistema
{
    public partial class frmServicioEgresos : Form
    {
        public frmServicioEgresos()
        {
            InitializeComponent();
        }
        ...
        {
            using (var form = new mdServicioIngreso(true, null, "GASTOS"))
            {
        ...
        private void mostrar_servicios()
        {
            ...
            string mensaje;
            List<Servicio> olista = ServicioLogica.Instancia.Listar(out mensaje).Where(s => s.TipoServicio == "GASTOS").ToList();
        ...
        private void dgdata_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (dgdata.Columns[e.ColumnIndex].Name == "btnEditar")
            {
                if (index >= 0)
                {
                    ...
                    using (var form = new mdServicioIngreso(false, oservicio, "GASTOS"))
                    {
                        var result = form.ShowDialog();
                        if (result == DialogResult.OK)
                        {
